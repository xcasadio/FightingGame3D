...
void CharacterCollision::SetHierarchy(LPD3DXFRAME pFrame_)
{
	...
	if (pFrameChild->pMeshContainer == nullptr)
	{
	...
	{
		IDirect3DVertexBuffer9* pVB_ = nullptr;
...
{
	auto pFrame = (D3DXFRAME_DERIVED*)(m_pFrame);

	if (m_pFrame->pFrameFirstChild == nullptr)
	{
		Window::Error(true, "cCharacterCollision::GetMatrix() : m_pFrame->pFrameFirstChild == NULL");
		return false;
	}

	auto pFrameChild = (D3DXFRAME_DERIVED*)(m_pFrame->pFrameFirstChild);
